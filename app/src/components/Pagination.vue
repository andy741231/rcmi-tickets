    ...
    <div v-if="totalPages > 1" class="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6">
        ...
        <div class="flex flex-1 justify-between sm:hidden">
            <button @click="go(page - 1)" :disabled="page <= 1"
                class="relative inline-flex items-center gap-1 rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-50">
                <Icon name="chevron-left" /> Prev
            </button>
            <button @click="go(page + 1)" :disabled="page >= totalPages"
                class="relative ml-3 inline-flex items-center gap-1 rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-50">
            ...
            <div>
                <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                    ...
                    <template v-for="(p, i) in visiblePages" :key="i">
                        <span v-if="p === '...'" class="relative inline-flex items-center border border-gray-300 bg-white px-4 py-2 text-sm text-gray-400">…</span>
                        <button v-else @click="go(p)"
                            ...
                            class="relative inline-flex items-center border px-4 py-2 text-sm font-semibold"
